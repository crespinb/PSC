    ...
    class FFTW
    {
        ...
        public:
            ...
            void fft( arma::vec& x, arma::cx_vec& Pxx)
            {
                //double*        in = x.memptr();
                //fftw_complex* out = reinterpret_cast<fftw_complex*>(Pxx.memptr());
                double* in = in_r.memptr();
                fftw_complex* out = reinterpret_cast<fftw_complex*>(out_c.memptr());
                if (pl_fft == NULL)
                {
            ...
            void ifft( arma::cx_vec& Pxx, arma::vec& x)
            {
                //fftw_complex* in = reinterpret_cast<fftw_complex*>(Pxx.memptr());
                //double*      out = x.memptr();
                fftw_complex* in = reinterpret_cast<fftw_complex*>(in_c.memptr());
                double* out = out_r.memptr();
                if (pl_ifft == NULL)
                {
